...
void botonAceptarNuevoMapaCB(int valor)
{
  ...
  if (ultimomapaPos >= 0)
  {
    ...
    if (ultimonivel == 4 or ultimonivel == 2)
    {
      obj2->enable();
      obj3->enable();
...
void botonConfigurarNuevoMapaCB(int valor)
{
  ...
  if (item > 0)
  {
    ...
    if (numObj == 3 and (nivel == 4 or nivel == 2) )
    {
      int f, c;
    ...
    if (numObj == 1 and (nivel == 4 or nivel == 2))
    {
...
void botonElegirMapaCB(int valor)
{
  ...
  panelSelecMapa->add_radiobutton_to_group(group, "Nivel 1: Optimo en coste (un objetivo)   ");
  panelSelecMapa->add_radiobutton_to_group(group, "Nivel 2: Optimo en coste (tres objetivos)");
...
void lanzar_motor_grafico(int argc, char **argv)
{
  ...
  // Main Window
  ventanaPrincipal = glutCreateWindow("Practica E1: Convocatoria Extraordinaria. Curso 21/22");
...
void lanzar_motor_grafico_verOnline(int argc, char **argv, EnLinea &argumentos)
{
  ...
  // Main Window
  ventanaPrincipal = glutCreateWindow("Practica E1: Convocatoria Extraordinaria. Curso 21/22. Version BAJO PARAMETROS");
  // Main Window callback function
  glutReshapeFunc(reshape);
  glutDisplayFunc(display_ventPrincipal);
